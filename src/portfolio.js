...
(function() {
    ...
    if (project.videos && project.videos.length > 0) {
        ...
        project.videos.forEach(video => {
            ...
            span.textContent = typeof label === 'object' ? (label[portfolioLang] || label.en) : label;
    ...
    if (project.links && Object.keys(project.links).length > 0) {
        ...
        if (project.links.github) {
            linksDiv.appendChild(createLinkButton('GitHub', project.links.github, createGitHubIcon()));
        }
        if (project.links.website) {
            linksDiv.appendChild(createLinkButton('Website', project.links.website, createWebsiteIcon()));
        }
        if (project.links.npm) {
            linksDiv.appendChild(createLinkButton('NPM', project.links.npm, createNPMIcon()));
        }
        if (project.links.jsr) {
            linksDiv.appendChild(createLinkButton('JSR', project.links.jsr, createJSRIcon()));
        }

        // Support for any other custom links
        Object.keys(project.links).forEach(key => {
            if (!['github', 'website', 'npm', 'jsr'].includes(key)) {
                linksDiv.appendChild(createLinkButton(
    ...
    if (project.description) {
        ...
        desc.textContent = typeof project.description === 'object'
    ...
    svg.setAttribute('viewBox', '0 0 24 24');
    svg.setAttribute('fill', 'currentColor');
    svg.innerHTML = '<path d="M0 7.334v8h6.666v1.332H12v-1.332h12v-8H0zm6.666 6.664H5.334v-4H3.999v4H1.335V8.667h5.331v5.331zm4 0v1.336H8.001V8.667h5.334v5.332h-2.669v-.001zm12.001 0h-1.33v-4h-1.336v4h-1.335v-4h-1.33v4h-2.671V8.667h8.002v5.331zM10.665 10H12v2.667h-1.335V10z"/>';
    return svg;
    }
    ...
    svg.setAttribute('viewBox', '0 0 24 24');
    svg.setAttribute('fill', 'currentColor');
    svg.innerHTML = '<path d="M3 3h18v18H3V3zm16 16V5H5v14h14zM7 7h10v2H7V7zm0 4h10v2H7v-2zm0 4h7v2H7v-2z"/>';
    return svg;
    }
